...
def load_reward_manager(config, tokenizer, num_examine, **reward_kwargs):
    ...
    use_format_reward = getattr(config.trainer, "format_reward", False)
    format_reward_coef = getattr(config.trainer, "format_reward_coef", 0.2)
    ...
    if compute_score is None:
        ...
        if sandbox_url:
            ...
            final_compute_score = partial(
                ...
                format_reward=use_format_reward,
                format_reward_coef=format_reward_coef,
            )
        else:
            final_compute_score = partial(
                default_compute_score,
